...
Original file is located at
    https://colab.research.google.com/drive/12VyMn-NBUKesq8f4WNW6sSGLchctzxR9
...
### Code
#### This work can be completed using basic Python, pySpark transformations and actions, and the plotting library matplotlib. Other libraries are not allowed.
### Files
#### Data files for this work are from the [metric-learning](https://code.google.com/p/metric-learning/) project, including:
...
amazonFullRecToToken = amazon.map(lambda name_product: (name_product[0], tokenize(name_product[1])))
googleFullRecToToken = google.map(lambda name_product: (name_product[0], tokenize(name_product[1])))
...
amazonWeightsRDD = amazonFullRecToToken.map(lambda key_value: (key_value[0], tfidf(key_value[1], idfsFullBroadcast.value)))
googleWeightsRDD = googleFullRecToToken.map(lambda key_value: (key_value[0], tfidf(key_value[1], idfsFullBroadcast.value)))
...
# TODO: Replace <FILL IN> with appropriate code
amazonNorms = amazonWeightsRDD.map(lambda key_value: (key_value[0], norm(key_value[1])))
amazonNormsBroadcast = sc.broadcast(amazonNorms.collectAsMap())
googleNorms = googleWeightsRDD.map(lambda key_value: (key_value[0], norm(key_value[1])))
...
def fastCosineSimilarity(record):
    ...
    tokens = record[1]
    s = reduce(lambda a, b: a + b, map(lambda token: amazonWeightsBroadcast.value[amazonRec][token] * googleWeightsBroadcast.value[googleRec][token], tokens))
    value = s / amazonNormsBroadcast.value[amazonRec] / googleNormsBroadcast.value[googleRec]
    key = (amazonRec, googleRec)
    return (key, value)

similaritiesFullRDD = (commonTokens
                       .map(lambda x : (x[0], list(x[1]))).map(lambda record: fastCosineSimilarity(record))
                       .cache())
